...
import { logProcessError, runCommand, runShell } from "../../../shared/utils/process"
...
async function getImageFileForId(id: string, ext: string): Promise<string | null> {
  ...
  try {
    ...
    logProcessError("clipvault image error", e)
  }
...
export async function loadClipvaultEntries(): Promise<ClipEntry[]> {
  let list = ""
  try {
    list = await runShell(`clipvault list | head -n ${itemsLimit}`)
  } catch (e) {
    logProcessError("clipvault list error", e)
    list = ""
  }
  ...
  for (const id of pinnedIds) {
    ...
    let txt = ""
    let detectedType: ClipTypes | "" = ""

    // Otherwise, ask clipvault for that id directly
    try {
      const line = await runShell(`clipvault list | grep -e '^${id}\t'`)
  ...
  return [...pinnedList, ...nonPinnedRecent]
}

export async function restoreClipToClipboard(id: string, opts?: { onRestore?: () => void }): Promise<void> {
  try {
    const list = await runCommand(["clipvault", "list"])
    ...
    runShell(cmd).catch((e) => {
      logProcessError("restoreClipToClipboard exec error", e)
    })
    await new Promise(r =>
    ...
    logProcessError("restoreClipToClipboard error", e)
  }
}
...
export async function deleteClip(id: string): Promise<void> {
  try {
    ...
    logProcessError("deleteClip error", e)
  }
}
...
function readPinnedIds(): string[] {
  try {
    ...
    logProcessError("readPinnedIds error", e)
    return []
  }
}
...
function writePinnedIds(ids: string[]) {
  try {
    ...
    logProcessError("writePinnedIds error", e)
  }
}
