...
export function NetworkWidget(cfg: NetworkWidgetConfig = {}) {
  ...
  root.add_css_class("network-root")
  if (cfg.windowLess ?? cfg.windowless) root.add_css_class("network-windowless")
  ...
  for (const [idx, entry] of resolvedSections.entries()) {
    if (totalSections === 1) {
      entry.section.controller.setPillClass(["network-pill-first", "network-pill-last"])
    } else if (idx === 0) {
      entry.section.controller.setPillClass("network-pill-first")
    } else if (idx === totalSections - 1) {
      entry.section.controller.setPillClass("network-pill-last")
    } else {
      entry.section.controller.setPillClass("network-pill-middle")
    }
    root.append(entry.section.controller.wrapper)
  }

  const footer = new Gtk.Box({ orientation: Gtk.Orientation.VERTICAL, spacing: 6 })
  footer.add_css_class("network-education")
  const footerLabel = new Gtk.Label({ label: "Education mode", xalign: 0 })
  footerLabel.add_css_class("network-education-label")
  ...
  footerHistoryBox.add_css_class("network-history")
  ...
  lastFooterLabel.add_css_class("network-footer-label")
  ...
  service.setActive("network-widget", false, refreshConfig)
  root.connect("map", () => {
    service.setActive("network-widget", true, refreshConfig)
  })
  root.connect("unmap", () => {
    service.setActive("network-widget", false, refreshConfig)
  })
